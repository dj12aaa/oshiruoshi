(() => {
  ...
  const items=[
    ['results','商品','#results'],['favorites','お気に入り',''],['compare','比較',''],['filter','条件',''],['saved','保存','#watch']
  ];
  const dock=document.createElement('nav');
  dock.id='utilityDock';dock.className='utility-dock';dock.setAttribute('aria-label','OSHIRU クイックメニュー');
  dock.innerHTML=`<a class="dock-home" href="/" aria-label="OSHIRU ホーム"><span>O</span></a><div class="dock-actions">${items.map(([key,label,href])=>href?`<a class="dock-item" href="${href}" data-dock="${key}">${icons[key]}<span>${label}</span>${key==='results'?'':'<b class="dock-count" hidden>0</b>'}</a>`:`<button class="dock-item" type="button" data-dock="${key}">${icons[key]}<span>${label}</span>${key==='filter'?'':'<b class="dock-count" hidden>0</b>'}</button>`).join('')}</div><div id="dockNotice" class="dock-notice" role="status" aria-live="polite"></div>`;
  document.querySelector('.site-header')?.insertAdjacentElement('afterend',dock);
  document.body.classList.add('dock-ready');

  const notice=$('#dockNotice');let noticeTimer;
  function showNotice(text){if(!notice)return;notice.textContent=text;notice.classList.add('show');clearTimeout(noticeTimer);noticeTimer=setTimeout(()=>notice.classList.remove('show'),1900)}
  ...
  function openFilters(){
    ...
    if(matchMedia('(max-width:900px)').matches){panel.classList.toggle('mobile-open');document.body.classList.toggle('filter-open',panel.classList.contains('mobile-open'));if(panel.classList.contains('mobile-open'))panel.scrollTop=0}
    else{panel.scrollIntoView({behavior:'smooth',block:'start'});panel.classList.add('filter-pulse');setTimeout(()=>panel.classList.remove('filter-pulse'),700)}
  }
  dock.addEventListener('click',e=>{
    const item=e.target.closest('[data-dock]');if(!item)return;
    const action=item.dataset.dock;
    if(action==='favorites'){e.preventDefault();$('#favoritesBtn')?.click();return}
    if(action==='compare'){
      e.preventDefault();const n=Number($('#compareCount')?.textContent||0);
      if(n>0)$('#openCompare')?.click();else{showNotice('比較したい商品の「＋」を押してください');smoothTo('#results')}return
    }
    if(action==='filter'){e.preventDefault();openFilters();return}
    if(action==='results'||action==='saved'){e.preventDefault();smoothTo(action==='results'?'#results':'#watch')}
  });

  function syncCounts(){
    const fav=Number($('#favCount')?.textContent||0),cmp=Number($('#compareCount')?.textContent||0);
    for(const [key,value] of [['favorites',fav],['compare',cmp]]){
      const b=dock.querySelector(`[data-dock="${key}"] .dock-count`);if(!b)continue;b.textContent=String(value);b.hidden=value<=0;
      dock.querySelector(`[data-dock="${key}"]`)?.classList.toggle('has-items',value>0);
    }
  }
  ['#favCount','#compareCount'].forEach(sel=>{const el=$(sel);if(el)new MutationObserver(syncCounts).observe(el,{childList:true,subtree:true,characterData:true})});
  syncCounts();

  document.addEventListener('click',e=>{
    if(!matchMedia('(max-width:900px)').matches)return;
    const panel=$('.filter-panel');if(!panel?.classList.contains('mobile-open'))return;
    if(e.target.closest('.filter-panel')||e.target.closest('[data-dock="filter"]')||e.target.closest('#mobileFilterBtn'))return;
    panel.classList.remove('mobile-open');document.body.classList.remove('filter-open');
  });
  ...
  function buildMobileCompare(){
    ...
    const wrap=document.createElement('section');wrap.className='mobile-compare';
    ...
    wrap.addEventListener('click',e=>{const btn=e.target.closest('[data-mobile-compare]');if(!btn)return;const group=btn.dataset.mobileCompare;wrap.querySelectorAll('[data-mobile-compare]').forEach(x=>x.classList.toggle('active',x===btn));wrap.querySelectorAll('.mobile-compare-field').forEach(x=>x.hidden=!(group==='all'||x.dataset.mobileGroup===group))});
  }
  const modalContent=$('#modalContent');if(modalContent)new MutationObserver(()=>queueMicrotask(buildMobileCompare)).observe(modalContent,{childList:true,subtree:true});
  buildMobileCompare();
})();
