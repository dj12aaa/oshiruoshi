...
### QL-024 指定語の欠落と広い種別への置換で検索精度が落ちる

- 症状: コラボ・場所・年など3語以上の追加条件のうち1語欠けた候補、またはボイスキー指定に通常のキーホルダーが通る。6c指定に16cが部分一致する。
- 根本原因: 完全一致filterに「3語なら1語欠落可」という候補取得用の緩和を残した。グッズ判定も取得用のbroad語を流用した。QL-019の「全件必須」という記録と実装が一致していなかった。
- 恒久対策: 明示語は全件必須、英数字の型番は境界一致、グッズは同義語のみ許可する。条件不一致を高scoreだけでfallback採用しない。商品名自体の一致を説明文だけの一致より優先する。
- 自動検査: 3条件それぞれの欠落、6c/16c/6cm、ボイスキー/通常キー、アクキー/金属キーを正負例として検査し、商品名一致の順位も比較する。
- 本番証拠: V19の正規APIに対する検索品質workflowとモバイル・PCの補正検索を確認する。現時点は43件のローカル回帰成功、本番V19確認前。

### QL-025 密度の維持を優先して必要な文字と検索状態まで読めなくする

- 症状: モバイル商品名9.5px、件数・補正語・並び替えが非表示。送料・状態も隠れ、0件と取得制限の違いが分からなかった。
- 根本原因: 「画像を小さく多列表示する」という要件を、結果説明と判断材料まで非表示にするCSSへ拡大解釈した。列数だけを検証し、文字の可読性と情報の到達性を検証しなかった。
- 恒久対策: 3列/7列と画像内操作は保持して商品名を12pxへ拡大。送料・状態、補正語・件数・並び替え、取得制限/絞り込みによる非表示件数を明示。重要CSSは既存の直読込fileで管理する。
- 自動検査: 本番browserで文字サイズ、summary/sort/detail可視、補正説明、320/390/768/1440/1920pxの列数と横overflowを検査する。
- 本番証拠: V19の正規URLで検査予定。CIだけで見やすさを達成済みとはしない。

### QL-026 不明な送料と破損した保存値を通常値として処理する

- 症状: 送料不明なら本体価格が上限を超えても残る。保存JSONが壊れると初期化時に例外となり検索イベントを登録できない。
- 根本原因: 価格上限を「総額が計算できる商品だけ」に適用し、localStorageを常に正常な配列JSONと仮定した。
- 恒久対策: 総額不明なら既知の本体価格に上限を適用し、送料不明を表示する。保存値は例外・型検査付きで読み、壊れた原本を勝手に削除せず空配列として安全に起動する。
- 自動検査: 送料込み超過・本体だけで超過・未知価格と、破損JSON/null/object/stringを実関数へ入力。browser起動時にも破損お気に入りとnull保存検索を設定して操作を確認する。
- 本番証拠: V19の本番browserで検索・連続検索・比較等の起動を確認予定。

### QL-027 観測していないSEO原因・CSP原因を断定する

- 症状: 外部検索で見えたURL数を全登録数のように扱い、被リンク不足を未計測のまま原因と記録した。ブラウザのCSP警告を本文hash照合前にJSON-LD由来と説明した。
- 根本原因: 観測値・仮説・確定原因を分離せず、改善実装から説明を逆算した。
- 恒久対策: Search Consoleの未確認事項を明記し、外部検索は露出サンプルとして扱う。CSP警告hashはJSON-LD本文と不一致であり、今回の接続ブラウザでは同警告も再現せず、原因未確定と訂正。根拠のないCSP許可追加を行わない。
- 自動検査: browserはconsoleの発生位置を保存し、初期DOMのJSON-LDを実際にparseする。SEO sourceと公開本文の検査を維持する。
- 本番証拠: 2026-09-06の公開画面でJSON-LDは610文字、検索動作を確認。Googleのインデックス理由・実トラフィック・被リンク数は未計測。V19では発見導線と利用者向けFAQも追加し、順位上昇は再crawl後の計測まで未確認とする。

### QL-028 編集用文字列置換が二重ドル記号を変換し初期化を停止する

- 症状: V19本番で`TypeError: $(...).forEach is not a function`が発生し、初回検索や後半の操作イベントが登録されなかった。
- 根本原因: 編集時にJavaScriptのString.replaceへ置換文字列として`$$('.query-chip')`を渡し、置換文字列の特殊構文`$$ → $`が働いた。構文とhelper単体検査は通ったが、app全体の初期化をローカルで実行していなかった。
- 恒久対策: 複数selectorを復元し、ドル記号を含む編集ではcallback置換で文字をそのまま返す。修正asset版を更新し旧cacheを排除する。
- 自動検査: `tests/usability-v19.test.mjs`でapp全体を最小DOM環境で実行し、例外なし、検索/並び替え/リセット/保存/画像検索のイベント登録、初回取得完了を検査する。本番browserのpageerrorもfail条件とする。
- 本番証拠: V19失敗run `34048991967`、job `101529152598`と公開Chrome consoleで同じ例外を確認。修正版V20の本番検証完了まで未解決として扱う。

### QL-029 設定の存在・過去確認・外部検索をライブ接続と混同する

- 症状: キーがあれば「接続済み」と表示し、楽天403でも成功のように見えた。多列UIでは確認時刻が隠れ、昔の情報が最新と誤解されやすかった。
- 根本原因: 実装済み・設定あり・検索時取得成功・API未接続を同じ表示へ集約した。Google Web検索と商品在庫API、Mercari Shopsの自店舗APIと全出品取得を区別する受入条件がなかった。
- 恒久対策: 7提供元のcapabilityを公開し、当該検索の取得結果と設定状態を分離。商品確認時刻を可視化。Yahoo広告IDを公式形式で検証して任意設定にし、順位への影響なし、未設定でも通常検索継続とする。未承認のアカウント・資格情報を勝手に作成しない。
- 自動検査: 秘密情報非露出、設定ありでもnot-checked、403/0件/外部検索の区別、広告有無で順位不変、片方API障害時の他方保持、過去日付保持を実行。ブラウザで連携ダイアログと7サイトの補正語リンクを検査。
- 本番証拠: V20正規URLで五条悟33件、なると26件、旧五条商品混入0件。楽天403は継続。V21の新しい連携表示・広告ID自動適用は本番検証待ちで、未設定広告IDの成果発生は未確認。

### QL-030 広告URLのquery部分を消す重複排除と未知価格の0円化

- 症状: 広告リンクを有効化すると、異なる商品でも同じredirect pathを持つため1件へ集約する潜在不具合があった。APIのnull/空値をNumber変換すると0円・送料無料になった。
- 根本原因: URLのqueryはすべて不要な追跡値と仮定し、nullを数値変換するJavaScript仕様を考慮しなかった。
- 恒久対策: 提供元＋商品ID／正規URLを優先して重複排除。未知価格・送料・在庫を推測しない。広告商品リンクはsponsoredを付け、明示する。
- 自動検査: 同じredirect pathの別商品2件を維持、同じIDの通常/広告URLを重複除去、null/空/負値を不明へ正規化。
- 本番証拠: V21検証待ち。外部広告IDの認可なしに実成果は確認しない。

### QL-031 モバイル検索ダイアログを無視した操作と広告fallbackのCSP違反

- 症状: V20 browser run `34141951583`で初回33件・3列・12px・overflowなしを確認後、背後の検索ボタンのclickがdialogに遮られた。広告のinline fallbackもCSPに遮断された。
- 根本原因: モバイル入力が専用dialogへ切り替わる仕様を検証手順へ反映しなかった。広告scriptを外部file必須のCSPと整合させなかった。
- 恒久対策: mobilePredictiveInputと「この言葉で検索」を操作し閉じるまで待つ。広告fallbackは外部同一origin JSへ移動し、unsafe-inlineは追加しない。
- 自動検査: workflowのdialog操作経路、広告HTMLに実行inline scriptがないこと、CSPを緩めていないことを検査。
- 本番証拠: V20の初期化例外は公開画面・browserログとも解消。V21で後続操作と広告fallbackを再検証する。

### QL-032 初回0件を最終0件と誤認し、数秒後の本検索と表示が矛盾する

- 症状: 検索直後に「条件に合う商品がありません」と表示され、数秒後に商品が出現した。
- 根本原因: render()が空配列を無条件で最終0件と解釈した。初回snapshotのfinallyがlive検索の未完了を見ずに検索ボタンと独立watchdogを解除した。成功時の最終画面しか検査せず、途中の誤表示を検出できなかった。
- 恒久対策: initial/live双方の状態から待機を判定し、どちらかが取得中なら0件を確定しない。届いた商品は即表示。両経路を含む12秒期限で停止を保証し、終了後・別検索の遅延応答は無視する。通信失敗は0件と区別し、検索語を保持した通常のYahoo!検索・Google検索をその場で案内する。
- 自動検査: snapshot先着0件→live成功、逆順、両方0件、初回/本検索失敗、既存商品の保持、全体期限、遅延/旧検索応答、空入力をVMで実行。Chromiumでも390/1440pxでliveを2秒保留し、0件誤表示0回と、成功/失敗/0件の6シナリオを検証する。結果はverification/v22-search-lifecycle.jsonへ保存する。
- 本番証拠: V22 main c5a8a92940100fba24d270477236fd2c8ca0c7c1、公開browser run 34183379623が2026-09-08に成功。390/1440pxで遅延成功・失敗・真の0件の6ケースすべて途中の誤0件表示0回。実検索「なると」は27件、モバイル3516ms/PC173ms、残留skeleton0・overflowなし。Yahoo/Google検索語保持も確認。外部APIの実通信待ちがゼロになったという意味ではない。

### QL-033 期限切れのhidden属性をカードのdisplay指定が上書きする

- 症状: latest-goods.jsは期限切れカードにhiddenを設定するが、カードCSSのdisplay:grid/flexがブラウザ標準のhidden表示を上書きできる状態だった。
- 根本原因: hidden属性だけを見て除外成功と扱い、実際の表示矩形を検査していなかった。
- 恒久対策: 最新情報ページの[hidden]をdisplay:none!importantで固定する。本文の確認日を勝手に最新化しない。
- 自動検査: 必須CSSの検査に加え、本番ブラウザで期限切れ要素のgetClientRects()が空であることを確認する。
- 本番証拠: V22公開browser run 34183379623で390/1440px双方visibleExpired=0、自動移動あり、overflowなし。

### QL-034 最新情報の画像が固定4件のままで、新しい販売元を取得しない

- 症状: スライドしても固定HTMLに埋め込んだ画像だけが表示され、各販売元から追加・更新されなかった。
- 根本原因: スライド操作の実装と情報更新の実装を混同し、最新情報ページに商品APIを取り直す経路がなかった。回帰検査も固定画像URLが残ることだけを検査していた。
- 恒久対策: 既存の編集確認欄と別に、公式商品API経由の画像を自動取得する5カテゴリのfeedを追加。テーマを15分単位で巡回、表示中5分更新、最大40件をカテゴリ・店舗間で交互表示。通常検索のrelevanceを共有し、snapshot・10分超・不明画像host・在庫なしを除外する。API画像のみを許可し、URL取得可能を転載許諾と誤認しない。取得時刻を新発売日として表示しない。未接続サイトは通常検索リンクを残す。
- 自動検査: tests/latest-feed-v23.test.mjsで許可元・URL境界・関連度・期間・店舗分散・重複・部分失敗・期限を検査。scripts/verify-latest-feed.cjsでPR上のfixtureと本番の実データを区別し、390/1440pxで画像・自動移動・停止・切替・失敗・広告表示・overflowを検証。Functionsは共有moduleにより11/12を維持。
- 本番証拠: V23の公開検証結果をverification/v23-feed-live.jsonとworkflowに保存する。ファイルが成功結果を持つまでは未確認。楽天の認可エラーや新規ニュース媒体の利用許諾はコード変更だけで解消したと扱わない。

### QL-035 検索で発見される対策と上位表示の成果を混同しない

- 症状: 検索で見つかりにくいという指摘に対して、メタタグや検索サンプルだけで順位原因を断定する危険があった。
- 根本原因: Search Consoleのインデックス状況・表示回数・検索語別順位を取得していない。最新ページは更新経路がなく、個別発表確認日とページ編集日も分けて説明されていなかった。
- 恒久対策: V18のrobots/sitemap/外部APIを待たないSEO本文を保持。最新ページに固有の更新方針・利用元・制限・比較ガイドへの内部リンクを静的HTMLで追加。変更したページだけlastmodを更新し、個別発表の確認日は捏造しない。
- 自動検査: SEO回帰とlatest-feedのHTML検査、公開HTTP検査。順位改善はこれらテストの合格条件にしない。
- 本番証拠: 2026-09-08の外部検索サンプルでトップとaboutを確認。これはGoogle全体の登録数や順位の測定ではない。Search Console権限・表示回数・CTR・平均順位の確認は未実施。V23のHTML配信は公開検証で確認する。

### QL-036 五条悟だけ複数の販売サイトがAPI接続済みに見える

- 症状: 五条悟ではメルカリ・Yahoo!フリマ等の商品が出る一方、ほかの商品では出ず、検索対象が限定されているように見えた。
- 根本原因: 保存した出品は全10件（メルカリ5、Yahoo!フリマ3、Yahoo!オークション2）で、五条悟・東雲絵名等に偏っていた。これらはverified/web-index-snapshotであり、当該サイトのlive APIではない。過去情報の件数が結果全体に含まれ、常時使える外部検索は一覧の末尾の折りたたみ内に隠れていた。
- 恒久対策: 過去情報の名称と含有件数を明示し、非APIサイトのfilterを「過去情報・外部検索」へ修正。「各サイトで検索」を結果近くに追加し、どの検索語でも8サイトへ現在の検索語を保持して移動できるようにする。未接続サイトをAPI接続済みと見せない。認可がないAPIを推測で接続しない。
- 自動検査: 五条悟・初音ミク・東雲絵名・未知の商品名で同じ8導線を生成するVM検査。本番の「なると」でも8導線の検索語保持とAPI未接続表記を検査。
- 本番証拠: V24の公開browser検査に記録。今回、メルカリ等の新規API認可・契約を取得したわけではない。

### QL-037 操作ガイドと取得元の説明がなく、利用者が迷う

- 症状: 検索・比較・画像検索・外部サイトへの移動が画面上から理解しにくく、過去情報と現在取得の違いも伝わらなかった。
- 根本原因: 機能追加に対して、実画面と対応した利用案内・再現可能な操作導線の検証を用意していなかった。
- 恒久対策: `/how-to-use`に実画面・番号・矢印・手順・FAQ・入力例を掲載。スクリーンショットの数字から説明へ移動し、その場の入力例から実検索へ遷移する。未設定の画像解析や未接続APIは明記。HTMLだけでも読める本文・form・リンクを基本にし、動的処理は例の選択だけに限定。
- 自動検査: 画像file・アンカー・canonical/sitemapと構造化データを検査。390/1440pxで画像、番号リンク、入力例、FAQ、検索への遷移、320/768pxで横overflowを検査。
- 本番証拠: V24の公開結果はverification/v24-guide.json・スクリーンショット・対象main SHAに記録する。順位改善の測定はSearch Console未接続のため未確認。

### QL-038 反映用の基準tree欠落で、変更対象外を削除扱いにする

- 症状: PR #24の初回差分が意図した29ファイルではなく117ファイルになり、既存API・CSS等が削除扱いになった。本番統合前の確認で停止した。
- 根本原因: GitHub tree作成前にbase_tree_shaの存在を必須検証せず、変更ファイルだけでroot treeを作成できてしまった。再開時の一時保存値を、検証済みの必須入力として扱った工程不備。
- 恒久対策: 現在のmainの完全なtreeを取得して基準を明示する。ref更新前に、実際に作成したtreeと基準tree・レビュー済みローカルmanifestを比較し、対象外の変更・削除・欠落・内容不一致を拒否する。追加修正で既存ファイルを復元し、PR全体の差分を再確認する。
- 自動検査: scripts/validate-release-tree.mjsを反映前に実行する。tests/release-tree.test.mjsで基準未指定、取得途中のtree、対象外削除・追加、内容不一致を拒否する。
- 本番証拠: 不完全な初回treeはmainへ統合していない。修正版のtree一致とPRの検査成功を確認してから公開する。
...
- V21確定証拠（2026-09-07）: product main 4be9e43166398079a015908de1e0dcbaa0d8e6e3。検索品質run 34143125270、browser run 34143125278が成功。モバイル/PCの3列/7列・12px商品名・確認日時・連携ダイアログ・外部7リンク・連続検索・画像検索・最新レールを確認。QL-024〜031の製品修正を公開確認済み。ただし広告ID未設定分の成果計測、楽天取得制限、Search Consoleは未解決。QL-032の途中0件表示はこの最終画面検査では見逃した。

- V18最終本番証拠: main `314a0590b216a3f8312123c32722b895fa6191bd`。HTTP run `33926095277`、検索品質 `33926095104`、browser `33926095157`はいずれも成功。390px/1440pxで3列/7列、検索停止・横overflowなし。QL-018〜023の公開配信と工程修正を確認。Google順位改善は別途未確認。
- 2026-09-06 V19候補: QL-024〜027を追加。ローカル43件成功。npm wrapper実行が環境側で中断されたため、同じscript本体の`node --test tests/*.test.mjs`で検証し、PR CIでは`npm test`も実行する。V19の本番状態は後続workflowの成果物で確認する。
