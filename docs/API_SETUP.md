...
2026-09-08追記: API障害・取得制限・0件の場合、結果見出し付近から同じ検索語で通常のYahoo!検索（search.yahoo.co.jp）/Google検索を開ける。Yahoo!ショッピングのAPIとは別の一般Web検索リンクであり、検索エンジンの結果をOSHIRUへ転載・自動収集するものではない。直接検索一覧も8経路へ拡張した。初回データが0件でも本検索が未完了なら「見つかりません」と表示しない。
