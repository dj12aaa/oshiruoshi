...
function withinPriceLimit(item,max){const known=total(item)??(item.price==null?null:Number(item.price));return known==null||!Number.isFinite(known)||known<=max}
const state={all:[],shown:[],direct:[],providers:{},query:'',resolvedQuery:'',favorites:new Set(readStoredArray('oshiru-v5-favs').filter(value=>typeof value==='string'||typeof value==='number').map(String)),compare:new Map(),broken:new Set(),loaded:new Set(),searchSeq:0};
...
function markSearch(patch={}){Object.assign(searchDiagnostics,patch,{updatedAt:new Date().toISOString()});renderSearchStatus()}
...
function applyFilters(){
  ...
  let arr=sources.size?state.all.filter(i=>{if(!sources.has(i.source))return false;if(type&&i.type!==type)return false;if(collab&&i.collab!==collab)return false;if(!includeEnded&&i.status==='終了')return false;if(liveOnly&&!itemLive(i))return false;if(!withinPriceLimit(i,max))return false;if(imageOnly&&(!i.image||i.imageVerified!==true||state.broken.has(String(i.id))))return false;return true}):[];
...
function openCompare(){const a=[...state.compare.values()];if(!a.length)return;$('#modalContent').innerHTML=`<h2>商品の比較</h2><p class="lead">商品価格だけでなく、送料と状態も合わせて比較します。</p><table class="compare-table"><tr><th>項目</th>${a.map(i=>`<th>${esc(i.source)}</th>`).join('')}</tr>${[['商品',i=>esc(i.title)],['価格',i=>fmt(i.price)],['送料',i=>fmt(i.shipping)],['実質額',i=>fmt(total(i))],['状態',i=>esc(i.condition||'要確認')],['販売状況',i=>esc(i.status||'要確認')],['確認',i=>esc(dt(i.verifiedAt))],['販売元',i=>`<a target="_blank" rel="${productRel(i)}" href="${esc(i.url)}">個別出品 ↗</a>`]].map(([h,f])=>`<tr><th>${h}</th>${a.map(i=>`<td>${f(i)}</td>`).join('')}</tr>`).join('')}</table>`;openModal()}
async function openSettings(){
  $('#modalContent').innerHTML='<h2>データ接続状況</h2><p>設定を確認しています…</p>';openModal();
  let s;try{s=await json('/api/status',{timeoutMs:3500})}catch{$('#modalContent').innerHTML='<h2>データ接続状況</h2><p>設定を取得できませんでした。時間をおいて再確認してください。</p>';return}
  const rows=(s.providers||[]).map(provider=>{
    const actual=state.providers[provider.id],label=providerStateLabel(provider,actual);
    const affiliate={configured:'広告リンク: ID設定あり（成果計測は未確認）','not-configured':'広告リンク: ID未設定',invalid:'広告リンク: ID形式を確認'}[provider.affiliate]||'';
    return `<div class="setting-row"><span>${esc(provider.name)}<small>${esc(provider.note||affiliate)}</small></span><b class="${actual?.ok===true?'status-on':'status-off'}">${esc(label)}</b></div>`;
  });
  rows.push(`<div class="setting-row"><span>過去の確認出品<small>現在の在庫を保証するものではありません。</small></span><b>${s.snapshot||0}件</b></div>`);
  rows.push(`<div class="setting-row"><span>画像AI検索</span><b>${s.vision?'設定あり・接続未検証':'API設定なし'}</b></div>`);
  $('#modalContent').innerHTML=`<h2>データ接続状況</h2><p class="lead">取得結果は今回の検索についての表示です。設定の存在だけでは接続成功と判定しません。</p><div class="settings-grid">${rows.join('')}</div><p class="integration-note">外部検索の結果はOSHIRUの件数に含めません。アフィリエイトの紹介料で検索順位は変更しません。登録・審査・規約同意は運営者による手続きが必要です。</p>`;
}
