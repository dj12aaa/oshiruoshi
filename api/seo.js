...
const pages={
  'guide/oshi-goods':{
    ...
    ],searches:[['推し活グッズを探す','推し活 グッズ'],['アクスタを探す','アクスタ'],['缶バッジを探す','缶バッジ'],['推しぬいを探す','推し ぬい'],['カードを探す','キャラクター カード']],related:['guide/how-oshiru-compares','compare/oshi-goods','compare/acrylic-stand','compare/can-badge','compare/plush','compare/card','discover/vtuber','discover/anime','discover/manga','discover/characters']
  },
  'guide/how-oshiru-compares':{
    path:'/guide/how-oshiru-compares',nav:'検索・比較の仕組み',title:'OSHIRUの検索・価格比較の仕組みと情報更新方針',description:'OSHIRUが推し活グッズをどのように検索・比較し、価格、送料、販売状況、情報の更新時点をどのように扱うかを説明します。',eyebrow:'SEARCH METHODOLOGY',h1:'OSHIRUの検索・比較は、何を基準にしているか。',lead:'検索結果を安心して使えるよう、取得元、検索順位の考え方、更新時点、確認が必要な情報を明示します。OSHIRUは販売サイトそのものではなく、複数の販売先を探し比べるための検索補助サービスです。',query:'推し活 グッズ',sections:[
      ['取得元を区別して表示する','Yahoo!ショッピングと楽天市場は利用可能な公式APIから商品候補を取得します。メルカリ、Yahoo!フリマ、Yahoo!オークションは同じ検索語で各販売サイトを確認できる直接導線を用意し、OSHIRU内に確認済み情報がある場合はその候補も表示します。'],
      ['検索語との一致を中心に並べる','確認済み候補は商品名、作品名、キャラクター名、グッズ種別、コラボ名などと検索語の一致を使って関連度を算出します。表示価格だけを理由におすすめ順位を上げる設計にはしていません。'],
      ['最新性には限界があるため販売元で確認する','価格、送料、在庫、商品状態、販売状況は取得・確認後に変わる可能性があります。OSHIRUでは取得時点を確認できる情報として扱い、購入・申込み前の最終判断は販売元の商品ページで行う前提です。'],
      ['広告枠と検索結果を分離する','楽天アフィリエイト等のPR表示は検索結果とは別枠で表示します。広告の有無を理由に、OSHIRUの検索結果の商品価格や関連度を変更しない方針です。']
  ...
  'compare/oshi-goods':{
    ...
    ],searches:[['推し活グッズ','推し活 グッズ'],['限定グッズ','推し グッズ 限定'],['コラボグッズ','推し コラボ グッズ'],['特典グッズ','推し 特典 グッズ']],related:['guide/how-oshiru-compares','compare/acrylic-stand','compare/can-badge','compare/plush','compare/card','discover/characters']
  },
  ...
  'discover/characters':{
    path:'/discover/characters',nav:'キャラ・作品から探す',title:'キャラクター・作品名から推しグッズを探す | OSHIRU',description:'VTuber、アニメ、漫画、バーチャルキャラクターなどの名前から推しグッズを探すOSHIRUの入口ページ。キャラクター名とアクスタ、缶バッジ、ぬい、カードなどを組み合わせて横断検索できます。',eyebrow:'CHARACTER & TITLE',h1:'キャラクター・作品名から、推しグッズを探す。',lead:'名前が決まっている場合は、キャラクター名やタレント名にグッズ種別を足す方法が最もシンプルです。代表ページと検索例から、そのままOSHIRU検索へ移動できます。',query:'推し キャラクター グッズ',sections:[
      ...
      ['固定ページは検索需要を見て増やす','OSHIRUでは薄いページを大量生成せず、検索されるテーマを確認しながら、役立つ情報を持つ固定ページを順次増やす方針です。']
...
<div class="seo-note">表示価格・送料・在庫・販売状況は取得時点の情報です。購入・申込み前に販売元の商品ページで最新情報、商品状態、付属品、送料等を必ず確認してください。各作品名・キャラクター名・タレント名・サービス名等の権利は各権利者に帰属し、OSHIRUは各権利者・販売サイトの公式サービスではありません。<a href="/guide/how-oshiru-compares">検索・比較の仕組みと更新方針</a>も確認できます。</div></main>
<footer class="site-footer"><div class="footer-inner"><div><div class="footer-brand">OSHIRU</div><div class="footer-note">推し活グッズを探す・比べる手間を少なくする横断検索サービスです。</div></div><nav class="seo-footer-links" aria-label="フッターナビゲーション"><a href="/guide/oshi-goods">グッズ比較ガイド</a><a href="/guide/how-oshiru-compares">検索・比較の仕組み</a><a href="/about.html">OSHIRUについて</a><a href="/terms.html">利用規約</a><a href="/privacy.html">プライバシー</a><a href="/disclaimer.html">免責・広告表示</a><a href="/contact.html">お問い合わせ</a></nav></div></footer></body></html>`}
