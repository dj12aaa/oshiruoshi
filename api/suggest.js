...
function primaryMerchGroups(intent){
  ...
  return groups.filter(group=>{const own=compactFlexible(group.canonical);return !groups.some(other=>other!==group&&compactFlexible(other.canonical).length>own.length&&compactFlexible(other.canonical).includes(own))});
}
function merchFragments(intent){
  ...
  for(const group of primaryMerchGroups(intent)){
    for(const value of [group.canonical,group.broad,...(group.aliases||[])].filter(Boolean)){
      ...
      for(let len=2;len<=Math.min(6,chars.length-1);len++){fragments.add(chars.slice(0,len).join(''));fragments.add(chars.slice(-len).join(''))}
    }
  }
  return fragments;
}
...
function correctKnownQuery(query=''){
  const tokens=normalizeFlexible(query).split(/\s+/).filter(Boolean),corrections=[];const corrected=tokens.map(token=>{const t=compactFlexible(token);if(LEXICON.some(x=>x.compact===t))return token;const kind=script(token);let best=null;for(const item of LEXICON){if(kind!=='mixed'&&script(item.alias)!==kind)continue;if(Math.abs([...t].length-[...item.compact].length)>2)continue;const score=similarity(t,item.compact);if(score<Math.max(threshold(token),threshold(item.alias)))continue;if(!best||score>best.score)best={item,score}}if(!best)return token;corrections.push({from:token,to:best.item.canonical,confidence:Number(best.score.toFixed(3))});return best.item.canonical});const value=corrected.join(' ').trim();return{query:String(query||''),corrected:value,changed:compactFlexible(value)!==compactFlexible(query),corrections};
}
...
export default async function handler(req,res){
  if(req.method!=='GET'){res.setHeader('Allow','GET');return send(res,405,{items:[]},30)}const u=new URL(req.url,'http://local'),q=clamp(u.searchParams.get('q')||'');if(normalizeFlexible(q).length<2)return send(res,200,{items:[]},180);const key=normalizeFlexible(q),cached=cache.get(key);if(cached&&Date.now()-cached.at<300_000)return send(res,200,cached.value,180);
  const prefix=completeKnownPrefix(q),known=correctKnownQuery(prefix.changed?prefix.corrected:q),preNormalized=known.changed?known.corrected:(prefix.changed?prefix.corrected:q),normalized=normalizeIntentQuery(preNormalized),baseQuery=normalized.cleaned||preNormalized,providerQuery=preferredProviderQuery(baseQuery),intent=detectIntent(baseQuery),groups=primaryMerchGroups(intent);
  ...
  if(normalized.changed)addCandidate(merged,{name:normalized.cleaned,kind:'correction',label:'重複・表記整理',detail:'グッズ名の重複や断片を整理',score:420});
  ...
  const canonical=preferredProviderQuery(baseQuery);if(canonical&&compactFlexible(canonical)!==compactFlexible(q))addCandidate(merged,{name:canonical,kind:'correction',label:'表記補正',detail:'略称・空白・表記ゆれを整理',score:340});for(const v of buildSearchVariants(baseQuery,4))if(['canonical','segmented'].includes(v.reason)&&compactFlexible(v.query)!==compactFlexible(q))addCandidate(merged,{name:v.query,kind:'correction',label:'入力補正',detail:'検索しやすい表記に整理',score:315+Number(v.weight||0)});
