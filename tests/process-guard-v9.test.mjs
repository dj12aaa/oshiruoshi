...
test('production verification waits for real cards and serializes result writers',()=>{
  ...
  for(const token of ['.product-card[data-card-id]','startup-diagnostics','requestfailed','startup search did not render merchandise','controlsInsideVisual','card overlays escaped the image area',"page.locator('#q').fill('なると')","page.locator('#searchBtn').click()",'diagnostics?.liveQuery===expectedQuery','search did not return live merchandise','SEARCH_UI_VERSION','HOME_EXPERIENCE_VERSION','AFFILIATE_UI_VERSION',"oshiru-search-ui-version\" content=\"2026-09-07.21",'oshiru-home-version','oshiru-affiliate-ui-version','verify-production-seo.sh','console.log(name,JSON.stringify(metrics))','if: always()','if-no-files-found: warn'])assert.ok(browser.includes(token),token);
  const http=read('.github/workflows/dense-latest-check.yml');
  for(const token of ['ready=0','OSHIRU gallery v9',"SEARCH_UI_VERSION='2026-09-07.21'",'oshiru-search-ui-version','verify-production-seo.sh','test "$ready" = 1'])assert.ok(http.includes(token),token);
  ...
  for(const token of ["import { MERCH_GROUPS, compactFlexible, normalizeFlexible }", "merchGroupOk('acrylic-stand'",'rankOrderOk'])assert.ok(search.includes(token),token);
